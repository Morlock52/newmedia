...
contract ContentOwnership is Ownable, ReentrancyGuard, Pausable {
    ...
    /**
     * @dev Publish content (make it publicly accessible)
     */
    function publishContent(uint256 contentId) external {
        ContentInfo storage contentInfo = content[contentId];
        if (contentInfo.creator == address(0)) revert ContentNotFound();
        if (contentInfo.creator != msg.sender && owner() != msg.sender) revert UnauthorizedAccess();
        if (contentInfo.status != ContentStatus.DRAFT) revert InvalidContentStatus();
        
        contentInfo.status = ContentStatus.PUBLISHED;
        contentInfo.updatedAt = block.timestamp;
        
        emit ContentStatusChanged(contentId, ContentStatus.DRAFT, ContentStatus.PUBLISHED);
    }
    ...
    function purchaseLicense(
        ...
        ContentInfo storage contentInfo = content[contentId];
        if (contentInfo.creator == address(0)) revert ContentNotFound();
        if (contentInfo.status != ContentStatus.PUBLISHED) revert InvalidContentStatus();
        if (!contentInfo.requiresLicense) revert InvalidLicenseTerms();
        
        LicenseTerms storage terms = licenseTerms[contentId];
        if (terms.priceUSD == 0) revert InvalidLicenseTerms();
        
        // Check if user already has valid license
        UserLicense storage existingLicense = userLicenses[contentId][msg.sender];
        if (existingLicense.isActive && 
            (existingLicense.expiresAt == 0 || existingLicense.expiresAt > block.timestamp)) {
            revert("License already active");
        }
        
        // Process payment
        paymentProcessor.payForContent(
            string(abi.encodePacked("content-", contentId)),
            contentInfo.creator,
            paymentToken
        );
        
        // Grant license
        uint256 expiresAt = terms.duration == 0 ? 0 : block.timestamp + terms.duration;
        
        userLicenses[contentId][msg.sender] = UserLicense({
            contentId: contentId,
            licensee: msg.sender,
            grantedAt: block.timestamp,
            expiresAt: expiresAt,
            licenseType: terms.licenseType,
            usesRemaining: terms.maxUses,
            isActive: true,
            licenseHash: terms.customTerms
        });
        
        userLicensedContent[msg.sender].push(contentId);
        totalLicenses++;
        
        emit LicenseGranted(contentId, msg.sender, expiresAt, terms.licenseType);
    }
    ...
    /**
     * @dev Access content (view or download)
     */
    function accessContent(
        ...
        uint32 requiredLicense = keccak256(bytes(accessType)) == keccak256(bytes("download")) 
            ? LICENSE_DOWNLOAD 
            : LICENSE_VIEW;
        
        if (!hasValidLicense(contentId, msg.sender, requiredLicense)) {
            revert UnauthorizedAccess();
        }
        
        // Update usage counts
        if (keccak256(bytes(accessType)) == keccak256(bytes("view"))) {
            contentInfo.viewCount++;
        } else if (keccak256(bytes(accessType)) == keccak256(bytes("download"))) {
            contentInfo.downloadCount++;
            
            // Decrement usage count if limited
            UserLicense storage userLicense = userLicenses[contentId][msg.sender];
            if (userLicense.isActive && userLicense.usesRemaining > 0) {
                userLicense.usesRemaining--;
                if (userLicense.usesRemaining == 0) {
                    userLicense.isActive = false;
                }
            }
        }
        
        emit ContentAccessed(contentId, msg.sender, accessType);
    }
    
    /**
     * @dev Set license terms for content
     */
    ...
    function _setLicenseTerms(uint256 contentId, LicenseTerms memory _licenseTerms) internal {
        ...
        // Set content price in payment processor
        if (_licenseTerms.priceUSD > 0) {
            paymentProcessor.setContentPrice(
                string(abi.encodePacked("content-", contentId)),
    ...
    function _getContentTypeFromMimeType(string memory mimeType) internal pure returns (MediaNFTOptimized.ContentType) {
        ...
        // Document types
        if (mimeHash == keccak256(bytes("application/pdf")) ||
            mimeHash == keccak256(bytes("text/plain"))) {
            return MediaNFTOptimized.ContentType.DOCUMENT;
        }
        
        // E-book types
        if (mimeHash == keccak256(bytes("application/epub+zip"))) {
            return MediaNFTOptimized.ContentType.EBOOK;
        }
    ...
    /**
     * @dev Emergency content removal
     */
    function removeContent(uint256 contentId, string memory reason) external onlyOwner {
        ContentInfo storage contentInfo = content[contentId];
        if (contentInfo.creator == address(0)) revert ContentNotFound();
        
        emit ContentStatusChanged(contentId, contentInfo.status, ContentStatus.REMOVED);
        contentInfo.status = ContentStatus.REMOVED;
        contentInfo.updatedAt = block.timestamp;
    }
}