...
/**
 * Complete MCP Server Base - Full 2025 Specification Support
...
class CompleteMCPServer {
  constructor(serverName, tools = [], resources = [], prompts = []) {
    this.serverInfo = {
      name: serverName,
      version: '1.0.0'
    };
    ...
    try {
      switch (request.method) {
        case 'initialize':
          ...
          // Add capabilities based on what we support
        ...
        case 'prompts/get':
          return await this.handlePromptGet(
            request.params.name,
            request.params.arguments || {}
          );
  ...
  start() {
    console.error(`[${this.serverInfo.name}] Starting Complete MCP Server...`);
