...
class MediaServerMCP extends UltraStableMCPServer {
  constructor() {
    const tools = [
      {
        name: 'search_media',
    ...
    const resources = [
      { uri: 'media://library', name: 'Media Library', mimeType: 'application/json' },
      { uri: 'media://stats', name: 'Library Statistics', mimeType: 'application/json' },
      { uri: 'media://system', name: 'System Information', mimeType: 'application/json' },
      { uri: 'media://activity', name: 'Recent Activity', mimeType: 'application/json' }
    ];

    super('media-server', tools, resources);
  }
  ...
  async handleResourceRead(uri) {
    ...
    const demoData = {
      ...
      'media://system': {
        services: {
          jellyfin: { status: 'running', version: '10.8.13' },
          sonarr: { status: 'running', version: '4.0.0.731' },
          radarr: { status: 'running', version: '5.2.6.8376' }
        }
      },
      'media://activity': {
        recent: [
          { type: 'movie', title: 'Oppenheimer', added: '2 hours ago' },
          { type: 'episode', title: 'The Last of Us S01E09', added: '5 hours ago' }
        ]
      }
    };
...
// Start the server
const server = new MediaServerMCP();
server.start();